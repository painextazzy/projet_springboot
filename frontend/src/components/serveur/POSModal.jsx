...
export default function POSModal({
  ...
  const validerCommande = () => {
    ...
    // ✅ Ouvre la confirmation dans le panier
  ...
  // Vue principale POS - Design mobile original
  return (
    <>
      {/* Toast Notification */}
      {notification.show && (
      ...
      {/* Modal de confirmation DANS le panier (pas de popup externe) */}
      {confirmationOpen && (
...
export default function POSModal({
  ...
  const ajouterAuPanier = async (plat) => {
    ...
    if (plat.quantite < quantite) {
      showNotification(`Stock insuffisant pour ${plat.nom}. Stock: ${plat.quantite}`, "error");
      return;
    }
  ...
  const platsFiltres = menu.filter((plat) => {
    if (categorieActive !== "TOUS" && plat.categorie !== categorieActive) return false;
    if (recherche && !plat.nom.toLowerCase().includes(recherche.toLowerCase())) return false;
    return plat.disponible;
  });
  ...
  if (showCart) {
    return (
      <div className="fixed inset-0 z-50 bg-surface-bright flex flex-col h-screen max-w-md mx-auto shadow-2xl overflow-hidden">
        <header className="pt-4 pb-2 px-6 flex justify-between items-center bg-surface-bright sticky top-0 z-50 border-b border-outline-variant/10">
          <div className="flex items-center gap-4">
            <button onClick={() => setShowCart(false)} className="material-symbols-outlined text-on-surface p-2 -ml-2 hover:bg-surface-container rounded-full transition-colors">
              arrow_back
            </button>
            <h1 className="font-headline font-extrabold text-xl tracking-tight text-on-surface">Votre Panier</h1>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-bold text-secondary uppercase tracking-wider">Table</span>
            <span className="font-headline text-xl font-extrabold text-on-surface leading-none">{table.nom || table.numero}</span>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6 pb-32">
          {panier.length === 0 ? (
            <div className="text-center py-12">
              <span className="material-symbols-outlined text-5xl text-slate-300 mb-3">shopping_basket</span>
              <p className="text-secondary">Votre panier est vide</p>
            </div>
          ) : (
            <div className="space-y-4">
              {panier.map((item) => (
                <div key={item.id} className="bg-surface-container-low rounded-xl p-4 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-headline text-base font-bold text-on-surface">{item.nom}</h3>
                      <p className="text-primary font-semibold text-sm">{formatPrix(item.prix)}</p>
                    </div>
                    <button onClick={() => supprimerDuPanier(item.id)} className="text-error hover:bg-error/10 p-2 rounded-full transition-all">
                      <span className="material-symbols-outlined text-base">delete</span>
                    </button>
                  </div>
                  <div className="flex justify-between items-center bg-surface-container-lowest rounded-lg p-2">
                    <div className="flex items-center gap-4">
                      <button onClick={() => modifierQuantitePanier(item.id, -1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-surface-container-highest text-on-surface hover:bg-outline-variant transition-colors">
                        <span className="material-symbols-outlined text-base">remove</span>
                      </button>
                      <span className="font-headline font-bold text-base min-w-[1.5rem] text-center">{item.quantite}</span>
                      <button onClick={() => modifierQuantitePanier(item.id, 1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-primary-container text-on-primary-container hover:bg-primary transition-colors">
                        <span className="material-symbols-outlined text-base">add</span>
                      </button>
                    </div>
                    <p className="font-headline font-extrabold text-on-surface">{formatPrix(item.prix * item.quantite)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {panier.length > 0 && !showConfirmationInCart && (
            <div className="mt-6 p-5 bg-surface-container-highest rounded-2xl">
              <div className="flex justify-between items-center mb-3">
                <span className="text-secondary font-medium">Sous-total</span>
                <span className="text-on-surface font-semibold">{formatPrix(calculerTotal())}</span>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-outline-variant/30">
                <span className="font-headline text-lg font-bold text-on-surface">Total à régler</span>
                <span className="font-headline text-xl font-extrabold text-primary">{formatPrix(calculerTotal())}</span>
              </div>
            </div>
          )}

          {/* ✅ Confirmation DANS le panier */}
          {showConfirmationInCart && (
            <div className="mt-6 p-5 bg-white rounded-2xl shadow-lg border border-primary/20">
              <div className="text-center mb-4">
                ...
                <h3 className="text-lg font-bold text-slate-900">Confirmer la commande</h3>
                <p className="text-slate-500 text-sm mt-1">
                  Valider cette commande ?<br />
                  <span className="font-bold text-primary text-lg">{formatPrix(calculerTotal())}</span>
                </p>
              </div>
        ...
        {/* Bouton "Demander l'Addition" - visible seulement si pas en confirmation */}
        {!showConfirmationInCart && panier.length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto px-6 pb-8">
            ...
            >
              <span className="material-symbols-outlined">receipt_long</span>
              <span className="font-headline font-bold text-base uppercase tracking-wider">Demander l'Addition</span>
            </button>
          </div>
        )}
      </div>
  ...
  return (
    <>
      {/* Toast Notification */}
      {notification.show && (
        <div className={`fixed top-16 left-1/2 -translate-x-1/2 z-[200] px-4 py-2 rounded-full shadow-lg transition-all duration-300 text-sm font-medium ${
          notification.type === "error" ? "bg-error text-white" : "bg-primary text-white"
        }`}>
          {notification.message}
        </div>
      )}
      ...
      <div className="fixed inset-0 z-50 bg-surface-bright flex flex-col h-screen max-w-md mx-auto shadow-2xl overflow-hidden">
        ...
        <header className="pt-4 pb-2 px-6 flex justify-between items-center bg-surface-bright sticky top-0 z-50">
          <div className="flex items-center gap-4">
            <button onClick={onClose} className="material-symbols-outlined text-on-surface p-2 -ml-2 hover:bg-surface-container rounded-full transition-colors">
              close
            </button>
            <h1 className="font-headline font-extrabold text-xl tracking-tight text-on-surface">Executive POS</h1>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-6 space-y-6 pb-32">
          ...
          <div className="grid grid-cols-2 gap-4 pb-4">
            {platsFiltres.length === 0 ? (
              <div className="col-span-2 text-center py-12 text-secondary">Aucun plat trouvé</div>
            ) : (
              platsFiltres.map((plat) => (
                <div key={plat.id} className="bg-white rounded-3xl p-3 shadow-sm border border-surface-container-high flex flex-col gap-3 group">
                  <div className="aspect-square rounded-2xl overflow-hidden bg-surface-container relative">
                    {plat.imageUrl ? (
                      <img
                        src={plat.imageUrl}
                        ...
                        onError={(e) => {
                          e.target.onerror = null;
                          e.target.src = "https://placehold.co/400x300/e2e8f0/64748b?text=🍽️";
                        }}
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-4xl bg-surface-container">🍽️</div>
                    )}
                    <div className="absolute bottom-2 right-2 flex items-center bg-primary text-on-primary rounded-lg px-2 py-1 text-xs font-bold shadow-lg">
                      {formatPrix(plat.prix)}
                    </div>
                  ...
                  <div className="flex flex-col px-1">
                    <h3 className="text-sm font-bold text-on-surface leading-tight">{plat.nom}</h3>
                    <p className="text-[11px] text-secondary font-medium">{getCategorieLabel(plat.categorie)}</p>
                  </div>
                  <div className="flex justify-end px-1 mb-0.5">
                    <span className={`text-[10px] font-medium ${plat.quantite <= 0 ? 'text-error' : plat.quantite <= 5 ? 'text-amber-600' : 'text-secondary/70'}`}>
                      Stock: {plat.quantite}
                    </span>
                  </div>
                  <div className="flex items-center justify-between bg-surface-container-low rounded-xl p-1 mt-1">
                    ...
                    <span className="text-sm font-bold px-2">{quantites[plat.id] || 1}</span>
                    <button
                      onClick={() => ajouterAuPanier(plat)}
                      disabled={plat.quantite <= 0}
                      className="material-symbols-outlined text-base p-1.5 bg-primary text-on-primary rounded-lg transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
