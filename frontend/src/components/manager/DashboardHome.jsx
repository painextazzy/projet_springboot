...
const DashboardHome = () => {
  ...
  const formatPrice = useCallback((price) => {
    return new Intl.NumberFormat("fr-MG").format(price || 0).replace(/\s/g, "") + " Ar";
  }, []);
  ...
  const calculateOccupancyRate = useCallback((tablesData) => {
    ...
    const occupiedStatuses = ["occupe", "occupée", "occupee", "occupied", "true", "1", "en cours", "utilisée", "prise", "OCCUPE", "OCCUPÉE", "OCCUPEE"];
  ...
  const calculateServiceMetrics = useCallback((commandesData) => {
    ...
    const lastPaidOrder = closedOrders.length > 0
      ? closedOrders.sort((a, b) => new Date(b.dateCloture) - new Date(a.dateCloture))[0]
      : null;

    const lastServiceTime = lastPaidOrder
      ? Math.floor((new Date(lastPaidOrder.dateCloture) - new Date(lastPaidOrder.dateOuverture)) / (1000 * 60))
      : 0;
    ...
    closedOrders.forEach((cmd) => {
      ...
      if (!lastOrderByTable[tableId] || cmdDate > new Date(lastOrderByTable[tableId].dateCloture)) {
    ...
    const averageServiceTime = serviceTimes.length > 0 
      ? Math.floor(serviceTimes.reduce((a, b) => a + b, 0) / serviceTimes.length)
      : 0;
  ...
  const calculateHourlyData = useCallback((commandesData) => {
    ...
    const maxRevenue = Math.max(...hours.map((h) => hourStats[h]?.revenue || 0), 1);
  ...
  const calculateDailyData = useCallback((commandesData) => {
    ...
    const maxRevenue = Math.max(...Object.values(dayStats).map((d) => d.revenue), 1);
  ...
  const calculateMonthlyData = useCallback((commandesData) => {
    ...
    const maxRevenue = Math.max(...Object.values(monthStats).map((m) => m.revenue), 1);
  ...
  const updateDashboardData = useCallback(async () => {
    try {
      ...
      const paidCommandes = filteredCommandes.filter(cmd => cmd.statut === "PAYEE");
      const totalRevenue = paidCommandes.reduce((sum, cmd) => sum + (cmd.total || 0), 0);
      const averageTicket = paidCommandes.length > 0 ? totalRevenue / paidCommandes.length : 0;
      
      const prevPaidCommandes = previousCommandes.filter(cmd => cmd.statut === "PAYEE");
      const prevRevenue = prevPaidCommandes.reduce((sum, cmd) => sum + (cmd.total || 0), 0);
      const revenueChange = prevRevenue > 0 ? ((totalRevenue - prevRevenue) / prevRevenue) * 100 : 0;
      const orderChange = previousCommandes.length > 0 ? ((filteredCommandes.length - previousCommandes.length) / previousCommandes.length) * 100 : 0;
      ...
      const top5 = Object.values(productSales).sort((a, b) => b.quantity - a.quantity).slice(0, 4);
      setTopProducts(top5);

      setOutOfStockProducts(menuData.filter(item => item.quantite === 0));
      setLowStockProducts(menuData.filter(item => item.quantite > 0 && item.quantite < 5));
  ...
  }, [timeFilter, calculateHourlyData, calculateDailyData, calculateMonthlyData, calculateOccupancyRate, calculateServiceMetrics]);
          ...
          <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-secondary font-body">
                ...
                <span className="text-primary font-medium">{getDateRangeText()}</span>
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="p-6 rounded-2xl bg-gradient-to-br from-primary to-primary-container text-white">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-white/70 text-xs uppercase tracking-wider">Chiffre d'Affaire</p>
                  <p className="text-2xl font-bold mt-1">{formatPrice(stats.dailyRevenue)}</p>
                  <p className={`text-xs mt-2 ${stats.dailyChange >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                    {stats.dailyChange >= 0 ? '+' : ''}{stats.dailyChange.toFixed(1)}% vs préc.
                  </p>
                </div>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 text-white">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-white/70 text-xs uppercase tracking-wider">Commandes</p>
                  <p className="text-2xl font-bold mt-1">{stats.totalOrders}</p>
                  <p className={`text-xs mt-2 ${stats.orderChange >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                    {stats.orderChange >= 0 ? '+' : ''}{stats.orderChange.toFixed(1)}% vs préc.
                  </p>
                </div>
                <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-700 text-white">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-white/70 text-xs uppercase tracking-wider">Ticket Moyen</p>
                  <p className="text-2xl font-bold mt-1">{formatPrice(stats.averageTicket)}</p>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-amber-600 to-amber-700 text-white">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-white/70 text-xs uppercase tracking-wider">Tables occupées</p>
                  <p className="text-2xl font-bold mt-1">{stats.occupancyRate}%</p>
                  <p className="text-xs text-white/50 mt-2">capacité actuelle</p>
                </div>
                <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 text-white">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-white/70 text-xs uppercase">Temps de service (dernière commande)</p>
                  <p className="text-3xl font-bold">{formatDuration(serviceMetrics.currentServiceTime)}</p>
                  <p className="text-white/60 text-xs mt-1">Moyenne par table: {formatDuration(serviceMetrics.averageServiceTime)}</p>
                </div>
              </div>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-rose-500 to-rose-600 text-white">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-white/70 text-xs uppercase">Commandes en attente</p>
                  <p className="text-3xl font-bold">{serviceMetrics.activeOrders}</p>
                </div>
              </div>
              <div className="text-white/70 text-sm">
          ...
          {orderDurationData.some((val) => val > 0) && (
            <div className="mb-8 p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/10">
              ...
              <div className="grid grid-cols-5 gap-3">
                {["0-15 min", "15-30 min", "30-45 min", "45-60 min", "60+ min"].map((label, index) => {
                  const maxValue = Math.max(...orderDurationData, 1);
                  const height = Math.max(20, (orderDurationData[index] / maxValue) * 100);
                  return (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 p-6 rounded-2xl bg-white border">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h4 className="text-lg font-bold">Évolution du Chiffre d'Affaires</h4>
                  <p className="text-secondary text-xs mt-1">par {getPeriodText().toLowerCase()}</p>
                </div>
              ...
              <div className="relative h-64">
                <svg className="w-full h-full" viewBox={`0 0 ${Math.max(currentData.length * 70, 600)} 250`} preserveAspectRatio="xMidYMid meet">
                  ...
                  <polyline
                    points={currentData.map((item, idx) => `${idx * 70 + 60},${230 - (item.revenue / maxRevenue) * 200}`).join(' ')}
                    fill="none"
                    stroke="#00307d"
                    strokeWidth="3"
                    strokeLinecap="round"
                  ...
                  {currentData.map((item, idx) => {
                    ...
                    return (
                      <g key={idx} className="group">
                        <circle cx={cx} cy={cy} r="5" fill="#00307d" stroke="white" strokeWidth="2" className="cursor-pointer transition-all hover:r-7" />
                        <rect x={cx - 35} y={cy - 28} width="70" height="22" rx="4" fill="#1f2937" className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                        <text x={cx} y={cy - 15} textAnchor="middle" fill="white" fontSize="9" fontWeight="bold" className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  ...
                  <text x="30" y="35" textAnchor="end" fontSize="9" fill="#9ca3af">{formatCompactPrice(maxRevenue)}</text>
                  <text x="30" y="85" textAnchor="end" fontSize="9" fill="#9ca3af">{formatCompactPrice(maxRevenue * 0.75)}</text>
                  <text x="30" y="135" textAnchor="end" fontSize="9" fill="#9ca3af">{formatCompactPrice(maxRevenue * 0.5)}</text>
                  <text x="30" y="185" textAnchor="end" fontSize="9" fill="#9ca3af">{formatCompactPrice(maxRevenue * 0.25)}</text>
                  <text x="30" y="235" textAnchor="end" fontSize="9" fill="#9ca3af">0</text>
                </svg>
                
                <div className="flex justify-between mt-2 text-[9px] text-secondary px-4">
                  {currentData.map((item, idx) => (
                    <div key={idx} className="text-center flex-1">{item.label}</div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-5 gap-2 mt-4 pt-3 border-t">
                {currentData.slice(-5).map((item, idx) => (
                  <div key={idx} className="text-center p-2 rounded-lg bg-gray-50">
                    ...
                    <p className="text-xs font-bold text-primary">{formatCompactPrice(item.revenue)}</p>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 text-white">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white/50 text-[9px] uppercase">Pic d'activité</p>
                    <p className="text-xl font-bold">
                      {currentData.reduce((max, item) => item.revenue > max.revenue ? item : max, currentData[0])?.label || "-"}
                    </p>
                    <p className="text-white/40 text-[8px] mt-0.5">
                      {formatPrice(currentData.reduce((max, item) => item.revenue > max.revenue ? item : max, currentData[0])?.revenue || 0)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white/50 text-[9px] uppercase">Affluence max</p>
                    <p className="text-xl font-bold">
                      {currentData.reduce((max, item) => item.count > max.count ? item : max, currentData[0])?.count || 0}
                    </p>
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
            {topProducts[0] && (
              <div className="lg:col-span-1 p-6 rounded-2xl bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20">
                <div className="flex items-center justify-between mb-3">
                  <span className="px-2 py-0.5 rounded-full bg-primary text-white text-[8px] font-bold uppercase">TOP VENTE</span>
                  <span className="material-symbols-outlined text-primary text-xl">emoji_events</span>
                </div>
                <h4 className="text-base font-bold mb-1 line-clamp-2">{topProducts[0].name}</h4>
                <p className="text-secondary text-xs">Vendu {topProducts[0].quantity} fois</p>
                <div className="mt-3 pt-3 border-t border-primary/20">
                  <div className="text-lg font-bold text-primary">{formatPrice(topProducts[0].revenue / topProducts[0].quantity)}</div>
                  <p className="text-[8px] text-secondary uppercase">Prix unitaire</p>
                </div>
              </div>
            )}
            ...
            <div className="lg:col-span-3 p-6 rounded-2xl bg-white border">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[9px] text-secondary">Par volume de ventes</span>
              </div>
                  ...
                  <tbody>
                    {topProducts.map((product, idx) => (
                      <tr key={idx} className="border-b border-gray-50">
                        <td className="py-3 text-center font-bold text-primary w-8">{idx + 1}</td>
                        <td className="py-3 font-medium">{product.name}</td>
                        <td className="py-3">{product.quantity} unités</td>
                        <td className="py-3">{formatCompactPrice(product.revenue)}</td>
                        <td className="py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-20 h-1 bg-gray-100 rounded-full overflow-hidden">
                              <div className="h-full bg-primary rounded-full" style={{ width: `${Math.min(100, (product.quantity / topProducts[0].quantity) * 100)}%` }}></div>
                            </div>
                            <span className="text-[9px] text-secondary">{Math.round((product.quantity / topProducts[0].quantity) * 100)}%</span>
                          </div>
                        </td>
                       </tr>
                    ))}
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-5 rounded-2xl bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200">
              ...
              <div className="space-y-1 max-h-36 overflow-y-auto">
                {outOfStockProducts.length > 0 ? outOfStockProducts.slice(0, 4).map((product) => (
                  <div key={product.id} className="flex justify-between items-center p-2 bg-white/80 rounded-lg">
                    <span className="text-sm font-medium">{product.nom}</span>
                    <span className="text-[10px] text-red-500 font-semibold bg-red-50 px-2 py-0.5 rounded-full">Épuisé</span>
                  </div>
                )) : (
                  <div className="text-center py-4 text-secondary text-sm">Aucun produit épuisé</div>
                )}
              </div>
            </div>
              ...
              <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-200">
                ...
                <div className="space-y-1 max-h-36 overflow-y-auto">
                  {lowStockProducts.slice(0, 4).map((product) => (
                    <div key={product.id} className="flex justify-between items-center p-2 bg-white/80 rounded-lg">
                      <span className="text-sm font-medium">{product.nom}</span>
                      <span className="text-[10px] text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded-full">Stock: {product.quantite}</span>
                    </div>
                  ))}
                </div>
              </div>
